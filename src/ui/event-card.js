...
export function renderEventCard(container, event) {
  ...
  container.innerHTML = `
    <div class="event-card event-${event.type}" data-quarter="${event.quarter}">
      <div class="event-card-icon">${meta.icon}</div>
      <div class="event-card-body">
        <div class="event-card-title">${meta.label}</div>
...
export function renderEventTimeline(container, events, currentQuarter) {
  ...
  if (quarterEvents.length === 0) {
    container.innerHTML = '<div class="event-timeline-empty">No events this quarter</div>'
    return
  }
  container.innerHTML = quarterEvents.map(e => `
    <div class="event-card event-${e.type}">
      ...
      <div class="event-card-body">
        <div class="event-card-title">${(TYPE_META[e.type] || {}).label || 'Event'}</div>
