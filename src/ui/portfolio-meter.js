export function createPortfolioMeter(container, playerId, engine) {
  ...
  meter.className = 'portfolio-meter'
  meter.innerHTML = `
    ...
    <div class="pm-stats">
      <div class="pm-stat">
        <span class="pm-stat-label">Return</span>
        <span class="pm-stat-value" id="pm-return">0.00%</span>
      </div>
      <div class="pm-stat">
        <span class="pm-stat-label">Impurity</span>
        <span class="pm-stat-value" id="pm-impurity">0.00%</span>
      </div>
      <div class="pm-stat">
        <span class="pm-stat-label">Allocated</span>
        <span class="pm-stat-value" id="pm-allocated">0%</span>
      </div>
      <div class="pm-stat">
        <span class="pm-stat-label">Volatility</span>
        <span class="pm-stat-value" id="pm-volatility">--</span>
      </div>
    </div>
    <div class="pm-bar-wrap">
      <div class="pm-bar-label">
        <span>Allocated</span>
        <span id="pm-alloc-pct">0%</span>
      </div>
      <div class="pm-bar">
        <div class="pm-bar-fill" id="pm-bar-fill" style="width:0%"></div>
      </div>
    </div>
  `
  ...
  return {
    update(state) {
      ...
      const capEl = meter.querySelector('#pm-capital')
      const retEl = meter.querySelector('#pm-return')
      const impEl = meter.querySelector('#pm-impurity')
      const allEl = meter.querySelector('#pm-allocated')
      const volEl = meter.querySelector('#pm-volatility')
      const barFill = meter.querySelector('#pm-bar-fill')
      const allocPct = meter.querySelector('#pm-alloc-pct')

      capEl.textContent = `$${Math.round(player.capital).toLocaleString()}`

      // Also update header capital
      ...
      const ret = player.initialCapital > 0
        ? (player.capital - player.initialCapital) / player.initialCapital
        : 0
      retEl.textContent = `${(ret * 100).toFixed(2)}%`
      retEl.className = 'pm-stat-value ' + (ret >= 0 ? 'positive' : 'negative')
      ...
      if (allocPct) allocPct.textContent = Math.min(pct, 100) + '%'

      const impurity = computeCurrentImpurity(player.currentAllocation, state.gameData.assets)
      impEl.textContent = `${(impurity * 100).toFixed(2)}%`
      impEl.className = 'pm-stat-value'
      if (impurity > 0.05) impEl.classList.add('negative')
      else if (impurity > 0.02) impEl.style.color = 'var(--warn)'
      else impEl.style.color = ''

      const vol = estimateVolatility(state)
...
function estimateVolatility(state) {
  if (!state.quarterPrices) return null
  const samples = []
